...
export class PostgresOrderRepository implements OrderRepository {
  ...
  //     const variantSizesToSearch: ORsearch[] = orderProducts.flatMap(
  //       (product) =>
  ...
  //     const tableVariantSizes = await tx.variantSize.findMany({
  //       where: {
  //         OR: variantSizesToSearch,
  //       },
  //       include: {
  //         size: {
  //           select: {
  ...
  //     const map = new Map<variantId_sizeValue, variantSizeId>();

  //     tableVariantSizes.forEach((variantSize) => {
  //       const key: variantId_sizeValue = `${variantSize.variantId}_${variantSize.size.sizeValue}`;
  //       map.set(key, variantSize.variantSizeId);
  //     });
  ...
  //       const orderProductId = UUID.generateRandomUUID().getValue();
  ...
  //           const variantSizeId = map.get(
  //             `${variant.variantId}_${variantSize.sizeValue}`
  //           );

  //           if (!variantSizeId) {
  //             throw new Error(
  //               `
  //                 Variant size not found for variantId: ${variant.variantId} and sizeValue: ${variantSize.sizeValue},
  //                 this should never happen, please check the database integrity.
  //                 If you are running this code in a test, ensure that the variant sizes are correctly seeded
  //               `
  //             );
  //           }
  ...
  //     await tx.orderVariant.createMany({
